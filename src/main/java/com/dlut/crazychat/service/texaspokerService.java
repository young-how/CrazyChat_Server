...
public class texaspokerService {
    ...
    private Integer currentPlayerIndex=0; // 当前玩家索引
    private Integer highestBet=0; // 当前最高下注金额
    private Integer pot=0; // 奖池金额
    private Integer currentTurn=0;  //当前下注轮次
    ...
    public void nextBettingRound() {
        List<texasPlayer> players = room;
        if (players == null || players.isEmpty()) {
            return;
        }
        int currentIndex = currentPlayerIndex;
        int currentHighestBet = highestBet;
        while (true) {
            ...
            if ((currentPlayer.hasActed() && currentPlayer.getCurrentBet() >= currentHighestBet)||currentPlayer.getMoney()==0) {
                ...
                // 这里是示例代码
                currentPlayer.act(currentHighestBet);

                // 更新奖池
                pot=pot + currentPlayer.getCurrentBet();
